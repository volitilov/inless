...
function includeFile(path, options) {
	...
	if(path.match(/^\/components/ig)) {
		path = './application'+path+'/markup/view/index.ejs';
		opts.filename = exports.resolveInclude(path, opts.compFilename);
	} else {
...
Template.prototype = {
	...
	generateSource: function() {
		...
		if (matches && matches.length) {
			...
			matches.forEach(function(line, index) {
				...
				var load = function(name, text) {
					var opts = utils.shallowCopy({}, self.opts);
					opts.__html = text;
					var path = require('path');
					var pth = path.resolve('./application/components/'+name+'/markup/view/index.ejs');
					if(fs.existsSync(pth)) {
						var includeSrc = includeSource(pth, opts);
						includeSrc = '    ; (function(){' + '\n' + includeSrc +
							'    ; })()' + '\n';
						self.source += includeSrc;
					} else {
						console.log('Error ejs: no find file:', pth);
					}
				}
				var testCustonInclude = function(line) {
					var data = null;
					if(data = line.match(/\<\/\$(\S+)\>/)) {
						if(inTag == data[1].toLowerCase()) {
							load(inTag, inTagData);
							// console.log('close');
							inTag = false;
							inTagData = '';
							return true;
					...
					if(inTag) {
						inTagData += line;
						// console.log('intag', line);
						return true;
					}
					if(data = line.match(/\<\$([^\/]+)\>/)) {
						// console.log('open', data);
						inTag = data[1].toLowerCase();
						return true;
					}
					if(data = line.match(/\<\$(\S+)\/\>/)) {
						// console.log('open&close', data);
						load(data[1].toLowerCase(), '');
						return true;
					}
					return false;
				}
				if(testCustonInclude(line)) {
					return;
				};
	...
	scanLine: function(line) {
		var self = this,
			d = this.opts.delimiter,
			newLineCount = 0;
		if(this.opts.__html.length) {
			var opts = utils.shallowCopy({}, self.opts);
			delete opts.__html;
			var templ = new Template(this.opts.__html, opts);
			line = line.replace(/\<Yield\/\>/ig, templ.compile());
		}
		function _addOutput() {
			if (self.truncate) {
				line = line.replace('\n', '');
